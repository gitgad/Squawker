...
public class FollowingPreferenceFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if(preference != null && preference instanceof SwitchPreferenceCompat){
            ...
            boolean isOn = ((SwitchPreferenceCompat) preference).isChecked();

            if(isOn){
                // Subscribe
                FirebaseMessaging.getInstance().subscribeToTopic(key);
                Log.d(LOG_TAG, "Subscribe to topic: " + key);
            } else {
                // Unsubscribe
                FirebaseMessaging.getInstance().unsubscribeFromTopic(key);
                Log.d(LOG_TAG, "Unsubscribe to topic: " + key);
            }
        }
    }
    ...
    public void onDestroy() {
        ...
        // Remove shared preferences change listener
